...
import fileinput
import Levenshtein
n_ratio = 10 #Negative to positive ratio
...
def make_train_val(X,y,tbl):
    ...
    n_goal_p = round(len(X)/(1/float(n_ratio+1))*n_split/100)
    n_goal_n = n_goal_p
...
for iteration in range(1, 30):
    ...
    model.fit(X_train, y_train, batch_size=BATCH_SIZE, nb_epoch=1, class_weight={1:10,0:1},validation_data=(X_val, y_val), show_accuracy=True)
