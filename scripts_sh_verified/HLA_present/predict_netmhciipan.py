...
def read_netmhc_xls(file0,list_run):
    file_out = open(file_name_in+'.temp','r')
    dict0 = dict()
    for line0 in file_out:
        ...
        if line0[1] in list_run:
            dict[line0[1]] = [float(line0[3]),float(line0[4]),float(line[5])]
    return dict0
...
def run_netmhciipan(hla_type_run,list_run,len_run):
    ...
    file_pep = open(file_name,'w+')
    file_pep.write(str_long)
    file_pep.close()    
    #run
    cmd_line = 'netMHCIIpan -f '+file_name_in+ ' -inptype 1 -a '+ \
    hla_type_run+ ' >'+file_name_in+'.temp' + ' -xls -xlsfile'+file_name_in+'.xls'
    print cmd_line
    cmd0 = subprocess.Popen(cmd_line,shell=True)      
    cmd0.wait() 
    #get data
...
def netmhcii_predict(hla_type0,list_seq):
    dict0 = dict()
