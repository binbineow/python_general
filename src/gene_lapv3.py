...
def main(pid):
    ...
    for type0 in ['MHC1','MHC2']:
        ...
        for x in ms_gene_list:
            if len(x.split('|'))==3:
                ms_gene_set.add(x.split('|')[1])
                #print x
            elif len(x.split('|'))==4:
                if x.split('|')[1] in gene_dict:
                    ms_gene_set.add(gene_dict[x.split('|')[1]])
                else:
                    ms_gene_set.add(x.split('|')[2])
                    counter.add(x.split('|')[2])
        ...
        '''
        print 'MCL'+pid+'\t'+type0+'\t'+str(len(counter))+'\t'+str(len(mut_gene_set & ms_gene_set))+'\t'+str(len(mut_gene_set - (mut_gene_set & ms_gene_set))) + '\t' + str(len(ms_gene_set - (mut_gene_set & ms_gene_set)))

    
#makegene_dict()
#main()
print 'patientID\tMHCIorII\tMSGenewithoutGeneID\tOverlapedGene\tGenes_predicted_only\tGenes_MS_only'
