...
path_Mut = '/scratch/users/bchen45/HLA_prediction/MCL_MHC_project/Neoantigen binding predictions/'
...
def main(pid):
    ...
    for type0 in ['MHC1','MHC2']:
        ...
        for i in range(0,len(ms_gene_list)):
            if ms_gene_list[i] in gene_dict:
                #generating a set
                ms_gene_set.add(gene_dict[ms_gene_list[i]])
            else:
                counter = counter +1
